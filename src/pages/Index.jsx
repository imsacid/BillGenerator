...
import { formatCurrency, convertAmount } from '../utils/formatCurrency'; // Corrected import path
...
const Index = () => {
  ...
  const handleCurrencyChange = (newCurrency) => {
    ...
    // Convert all item amounts to the new currency
    const convertedItems = items.map(item => ({
      ...item,
      amount: Number(convertAmount(item.amount, oldCurrency, newCurrency).toFixed(2)),
      total: Number(convertAmount(item.amount * item.quantity, oldCurrency, newCurrency).toFixed(2))
    }));
